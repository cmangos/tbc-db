...
(2562, 1, 1, 'Gubber, I don\'t have a fishing pole.', 5530, 3, 128, 0, 0, 256201, 0, 0, NULL, 0, 814); -- npc_option_npcflag 4 classic

DELETE FROM `conditions` WHERE `condition_entry` BETWEEN 810 AND 814;
...
-- Correct Gossip For Sputtervalve - renumber
UPDATE `gossip_menu_option` SET `action_menu_id` = 1381 WHERE `menu_id` = 21 AND `id` = 1; -- 20025
UPDATE `gossip_menu` SET `entry` = 1381 WHERE `entry` = 20025;
...
INSERT INTO `gossip_menu_option` (`menu_id`, `id`, `option_icon`, `option_text`, `option_broadcast_text`, `option_id`, `npc_option_npcflag`, `action_menu_id`, `action_poi_id`, `action_script_id`, `box_coded`, `box_money`, `box_text`, `box_broadcast_text`, `condition_id`) VALUES
(1922, 1, 1, 'I\'d like to purchase more Tharlendris seeds.', 4845, 3, 128, 0, 0, 0, 0, 0, '', 0, 965); -- npc_option_npcflag 4 instead of 128 classic + condition Quest ID 3785 Rewarded
...
-- UPDATE `gossip_menu_option` SET `npc_option_npcflag` = 4 WHERE `menu_id` IN (1701,1922); -- classic, one was wrong!
