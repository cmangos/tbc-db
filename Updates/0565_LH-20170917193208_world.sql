...
UPDATE `game_event` SET `description` = 'Elemental Invasion: Water' WHERE `entry` = 38;
-- Align Rifts
UPDATE `gameobject` SET `spawntimesecsmin` = 300, `spawntimesecsmax` = 300, `animprogress` = 255 where `guid` IN (SELECT `guid` FROM `game_event_gameobject` where `event` = 13);
UPDATE `gameobject` SET `animprogress` = 255 where `guid` IN (SELECT `guid` FROM `game_event_gameobject` where `event` = 14);
UPDATE `gameobject` SET `animprogress` = 255 where `guid` IN (SELECT `guid` FROM `game_event_gameobject` where `event` = 15);
UPDATE `gameobject` SET `animprogress` = 255 where `guid` IN (SELECT `guid` FROM `game_event_gameobject` where `event` = 38);
...
UPDATE `creature` SET `position_x` = -5310.00, `position_y` = 610.82, `position_z` = 390.2687, `spawndist` = 3 WHERE `guid` = 2470 AND `id` = 8503;
UPDATE `creature` SET `position_x` = -5070.83, `position_y` = 500.00, `position_z` = 401.4849, `spawndist` = 3 WHERE `guid` = 98662 AND `id` = 8503;
