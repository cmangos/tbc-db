...
-- TBC
UPDATE `creature_template` SET `DamageSchool` = 5 WHERE `entry` = 21941; -- Accursed Apparition
UPDATE `creature_template` SET `SchoolImmuneMask` = 16 WHERE `entry` = 17155; -- Lake Surger
UPDATE `creature_template` SET `SchoolImmuneMask` = 4 WHERE `entry` = 25001; -- Abyssal Flamewalker
UPDATE `creature_template` SET `DamageSchool` = 0 WHERE `entry` IN (24777,25571); -- Sunblade Sentinel
UPDATE `creature_template` SET `DamageSchool` = 0 WHERE `entry` IN (24554,25550); -- Eramas Brightblaze

-- Poison Immunity Research
REPLACE INTO `creature_immunities` SELECT `entry`, 0, 4, 4 FROM `creature_template` WHERE `entry` IN (
15691, -- The Curator (confirmed)
19516, -- Void Reaver
