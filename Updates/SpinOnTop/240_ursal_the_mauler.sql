...
(@ENTRY,1,25,1,0,2852,46212,16,0,0,0,0,0,0,0,0,'Enslaved Druid of the Talon 1 - run on'),
(@ENTRY,1,25,1,0,2852,46213,16,0,0,0,0,0,0,0,0,'Enslaved Druid of the Talon 2 - run on'),
(@ENTRY,1,25,1,0,2852,46214,16,0,0,0,0,0,0,0,0,'Enslaved Druid of the Talon 3 - run on'),
(@ENTRY,1,25,1,0,2852,46215,16,0,0,0,0,0,0,0,0,'Enslaved Druid of the Talon 4 - run on'),
(@ENTRY,2,0,0,0,2852,46212,16,2000002852,0,0,0,0,0,0,0,'Enslaved Druid of the Talon 1 - say line 1'),
...
DELETE FROM `db_script_string` WHERE `entry`=2000002852;
INSERT INTO `db_script_string` (`entry`, `content_default`, `comment`) VALUES 
(2000002852, 'The nightmare of Ursal is over! We are free!', 'Enslaved Druid of the Talon');
...
(@ENTRY, 2, 9079.52, 1872.34, 1334.99, 0),
(@ENTRY, 3, 9024.94, 1885.46, 1334.4, 0),
(@ENTRY, 4, 9024.94, 1885.46, 1334.4, @ENTRY*100+01);

DELETE FROM `dbscripts_on_creature_movement` WHERE `id`=@ENTRY*100+01;
