...
-- c.guid.81379 was deleted in tbcmangos prior to this commit. duplicated 81381 prev.
DELETE FROM `creature` WHERE `guid` = 81379 AND `id` = 822;
INSERT INTO `creature` (`guid`, `id`, `map`, `position_x`, `position_y`, `position_z`, `orientation`, `spawntimesecsmin`, `spawntimesecsmax`, `spawndist`, `movementtype`) VALUES
(81379, 822, 0, -9447.15, -1474.35, 61.8309, 6.17525, 180, 180, 5, 1);

UPDATE `creature` SET `spawntimesecsmin` = 180, `spawntimesecsmax` = 180, `spawndist` = 5, `MovementType` = 1 WHERE `id` = 822; -- general cleanup due to tbc having additional nodes
UPDATE `creature` SET `spawndist` = 15 WHERE `guid` = 80480 AND `id` = 822;
...
DELETE FROM `creature` WHERE `guid` IN (99875,99890); -- now duplicates after moving

-- Prowler 118 (diff down to +/-7)
-- duplicates
DELETE FROM creature_addon WHERE guid IN (81431,81376); -- d.81403 (creature_spawn_entry?)
...
-- update
-- UPDATE creature SET spawndist = 10, MovementType = 1 WHERE id = 118; -- no idea why we had some with spawndist = 0 tbcmangos has 15 already
UPDATE creature SET position_x = -9482.156250, position_y = -1442.788208, position_z = 59.370399 WHERE guid = 81399; -- close to 81397
