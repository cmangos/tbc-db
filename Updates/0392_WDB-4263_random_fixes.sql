...
DELETE FROM `questgiver_greeting` WHERE `Entry` in (1740,1938,1952,2055,2229,2277,2500,2934,3050,3428,4454,4630,5598,5636,5675,5878,6868,7802,10428,11259,11596,14567,19936); -- 23 max (spawns exist)
...
-- Blood Elf Rogue only
UPDATE quest_template SET RequiredRaces = 512 WHERE entry IN (9532, 9460, 9618); -- all races, horde obv.

-- Draenei shaman only (not really necessary, Alliance has only shamans for Draenei)
UPDATE quest_template SET RequiredRaces = 1024 WHERE entry IN (9449, 9450, 9451); -- had all alliance races

-- Start class quests for Blood Elf only
UPDATE quest_template SET RequiredRaces = 512 WHERE entry IN (8328, 8563, 8564, 9392, 9393, 9676); -- had all horde races

-- Start class quests for Draenei only , missing pre-quest
UPDATE quest_template SET RequiredRaces = 1024 , PrevQuestId = 9280 WHERE entry IN (9287, 9288, 9289, 9290, 9291, 9421); -- https://tbc.wowhead.com/quest=9280/replenishing-the-healing-crystals
-- https://tbc.wowhead.com/quest=9421/shaman-training#comments - had all races

-- missing Draenei Priest Questgiver/taker (Priest Training)
DELETE FROM creature_questrelation WHERE quest = 9291;
INSERT INTO creature_questrelation (id, quest) VALUES (16502, 9291);
DELETE FROM creature_involvedrelation WHERE quest = 9291;
INSERT INTO creature_involvedrelation (id, quest) VALUES (16502, 9291);
...
(142706, 2933),
(142707, 2933),
(142708, 2933), -- not spawned
(142709, 2933), -- not spawned
(142710, 2933), -- not spawned
(142711, 2933), -- not spawned
(142712, 2933),
(142713, 2933),
(142714, 2933);
